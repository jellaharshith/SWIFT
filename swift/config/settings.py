...
def get_config() -> Config:
    ...
    Reads .env file via python-dotenv, then overlays actual environment
    variables. Raises immediately if required keys are absent or values
    are out of range — this surfaces misconfiguration at startup rather
    than mid-scan.
...
def reset_config() -> None:
    ...
    Intended exclusively for use in tests — production code should never
    call this. Without resetting between tests, the module-level cache
    would cause tests to share state and produce false passes/failures.
    """
    global _config
    _config = None
